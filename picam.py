...
filepathVideo = "/home/pi/picam_video"
...
def saveVideo(settings, width, height, quality, diskSpaceToReserve):
    keepDiskSpaceFree(diskSpaceToReserve)
    time = datetime.now()
    filename = filepathVideo + "/" + filenamePrefix + "-%04d%02d%02d-%02d%02d%02d" % (time.year, time.month, time.day, time.hour, time.minute, time.second)
    subprocess.call("raspivid -o %s -t 20000 -w 640 -h 480" % ( filename+'.h264'), shell=True)
    print("VIDEO: "+filename+".h264")

    subprocess.call("MP4Box -add %s %s" % ( filename+'.h264', filename+'.mp4'), shell=True)
    subprocess.call("rm %s" % ( filename+'.h264'), shell=True)
...
while (True):
    ...
    if takePicture:
        ...
        saveVideo(cameraSettings, saveWidth, saveHeight, saveQuality, diskSpaceToReserve)
